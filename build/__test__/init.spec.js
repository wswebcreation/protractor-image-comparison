...
describe('protractor-image-comparison local development initialization', () => {
    ...
    it('should save the compare screenshot screenshots', () => {
        browser.imageComparson.saveScreen(examplePage)
            .then(() => {
            fs.copySync(`${screenshotPath}/${examplePage}-${logName}-${resolution}.png`, `${baselineFolder}/${examplePage}-${logName}-${resolution}.png`);
            fs.copySync(`${screenshotPath}/${examplePage}-${logName}-${resolution}.png`, `${baselineFolder}/${examplePageFail}-${logName}-${resolution}.png`);
        });
    });
    it('should save the default compare element screenshots', () => {
        browser.executeScript('arguments[0].scrollIntoView();', dangerAlert.getWebElement())
            ...
            .then(() => {
            fs.copySync(`${screenshotPath}/${dangerAlertElement}-${logName}-${resolution}.png`, `${baselineFolder}/${dangerAlertElement}-${logName}-${resolution}.png`);
            fs.copySync(`${screenshotPath}/${dangerAlertElement}-${logName}-${resolution}.png`, `${baselineFolder}/${dangerAlertElementFail}-${logName}-${resolution}.png`);
            fs.copySync(`${screenshotPath}/${dangerAlertElement}-${logName}-${resolution}.png`, `${baselineFolder}/${dangerAlertElementFail}-ignore-antialiasing-${logName}-${resolution}.png`);
        });
    });
    it('should save the resized compare element screenshots', () => {
        browser.imageComparson.saveElement(headerElement, 'resizeDimensions-header-element', { resizeDimensions: 15 })
            .then(() => {
            fs.copySync(`${screenshotPath}/resizeDimensions-header-element-${logName}-${resolution}.png`, `${baselineFolder}/resizeDimensions-header-element-${logName}-${resolution}.png`);
            ...
            .then(() => {
            fs.copySync(`${screenshotPath}/resizeDimensions-${dangerAlertElement}-${logName}-${resolution}.png`, `${baselineFolder}/resizeDimensions-${dangerAlertElement}-${logName}-${resolution}.png`);
        });
    });
    it('should save the compare fullpage screenshots', () => {
        browser.imageComparson.saveFullPageScreens(exampleFullPage)
            .then(() => {
            fs.copySync(`${screenshotPath}/${exampleFullPage}-${logName}-${resolution}.png`, `${baselineFolder}/${exampleFullPage}-${logName}-${resolution}.png`);
            fs.copySync(`${screenshotPath}/${exampleFullPage}-${logName}-${resolution}.png`, `${baselineFolder}/${exampleFullPageFail}-${logName}-${resolution}.png`);
        });
    });
});
