...
export async function getBase64FullPageScreenshotsData(takeScreenshot, executor, options) {
	...
	if (isAndroid && isAndroidNativeWebScreenshot) {
		...
		const { height: statusAddressBarHeight } = (await executor(getAndroidStatusAddressToolBarHeight, OFFSETS.ANDROID)).statusAddressBar;
		...
		const { height: statusAddressBarHeight } = (await executor(getIosStatusAddressToolBarHeight, OFFSETS.IOS)).statusAddressBar;
