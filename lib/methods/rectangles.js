...
export async function determineStatusAddressToolBarRectangles(executor, options) {
	const { blockOutStatusBar, blockOutToolBar, isMobile, platformName, isAndroidNativeWebScreenshot } = options;
	const rectangles = [];

	if (isMobile && (checkAndroidNativeWebScreenshot(platformName, isAndroidNativeWebScreenshot) || checkIsIos(platformName))) {
