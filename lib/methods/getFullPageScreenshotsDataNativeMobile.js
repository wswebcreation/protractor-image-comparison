 ...
 * @param  {number}    statusAddressBarHeight The height of the status and address bar
 ...
 * }>}
 */
export default async function getFullPageScreenshotsDataNativeMobile(
	takeScreenshot,
	executor,
	...
	for (let i = 0; i <= amountOfScrollsArray.length; i++) {
		...
		screenshotSizeWidth = getScreenshotSize(screenshot).width;
		...
		viewportScreenshots.push({
			...
			imageHeight: imageHeight * devicePixelRatio,
			imageWidth: screenshotSizeWidth,
			imageYPosition: imageYPosition * devicePixelRatio,
			screenshot,
		});
	}

	return {
		fullPageHeight: (scrollHeight - addressBarShadowPadding - toolBarShadowPadding) * devicePixelRatio,
