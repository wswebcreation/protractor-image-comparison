...
import determineRectangles from "../methods/determineRectangles";
...
export default async function saveScreen(methods, instanceData, folders, tag, options) {
  ...
  await beforeScreenshot(methods.executor);
  ...
  const rectangles = determineRectangles(screenshot, enrichedInstanceData);
  ...
  await afterScreenshot(methods.executor);
