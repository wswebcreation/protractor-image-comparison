...
function resembleJS(fileData, fileTwo, options) {
        ...
        for (i = distance * -1; i <= distance; i++) {
            for (j = distance * -1; j <= distance; j++) {
                ...
                if (i === 0 && j === 0) {
                    ...
                    targetPix = _getPixelInfo(data, offset, cacheSet);
    ...
    function _errorPixel(px, offset, data1, data2) {
    ...
    function _getPixelInfo(data, offset, cacheSet) {
        var r;
        var g;
        var b;
        var d;
    ...
    function _analyseImages(img1, img2, width, height) {
        ...
        _loop(height, width, function (verticalPos, horizontalPos) {
            ...
            var pixel1 = _getPixelInfo(data1, offset, 1);
            var pixel2 = _getPixelInfo(data2, offset, 2);
            ...
            if (ignoreColors) {
                ...
                if (_isPixelBrightnessSimilar(pixel1, pixel2)) {
                    ...
                    _errorPixel(targetPix, offset, pixel1, pixel2);
                    mismatchCount++;
    ...
    return {
        onComplete: function (callback) {
            ...
            return _compare(fileData, fileTwo);
        }
    };
